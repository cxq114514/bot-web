...
import {
  removeProfile, lastProfileStorageKey, profileStorageKey,
  persistentConfig, type SiteProfile,
} from "../lib/profiles";

test("base href resolves fields and pagination without changing the source URL", () => {
  const source = "https://example.com/index.html";
  const html = '<base href="/catalogue/"><base href="/ignored/"><article class="product_pod"><h3><a href="book.html">Book</a></h3></article><li class="next"><a href="page2.html">Next</a></li>';
  const result = extractPage(html, bookConfig, source);
  assert.equal(result.rows[0]["详情链接"], "https://example.com/catalogue/book.html");
  ...
  for (const base of ["http://[", "javascript:alert(1)"]) {
    const fallback = extractPage(html.replace("/catalogue/", base), bookConfig, source);
...
test("page warnings reach the client before a later failure or cancellation", async () => {
  for (const cancel of [false, true]) {
    ...
    await assert.rejects(crawl(
      { ...bookConfig, maxPages: 2 }, aborter.signal,
      (event) => {
        events.push(event);
      ...
      },
      {
        get: async (url, _signal, before) => {
          ...
          return {
            ...
            body: Buffer.from('<article class="product_pod"></article><li class="next"><a href="?page=2">Next</a></li>'),
          };
        },
        request: async () => { throw new Error("unexpected API call"); },
      },
      async () => {},
    ));
    assert.equal(events.filter((e) => e.type === "page").length, 1);
    assert.equal(events.filter((e) => e.type === "log" && e.level === "warning").length, 4);
...
test("cross-origin base pagination remains blocked by crawler", async () => {
  const result = await crawl(
    { ...bookConfig, maxPages: 2 }, new AbortController().signal, () => {},
    {
      get: async (url, _signal, before) => {
        if (url.endsWith("/robots.txt"))
          return { url, status: 404, headers: {}, body: Buffer.from("") };
        ...
        return {
          url, status: 200, headers: { "content-type": "text/html" },
          body: Buffer.from('<base href="https://other.example.com/"><li class="next"><a href="next">Next</a></li>'),
        };
      },
      request: async () => { throw new Error("unexpected API call"); },
    }, async () => {},
...
test("deleting A preserves B auto-restore; deleting B clears it, including legacy storage", () => {
  const profiles: SiteProfile[] = ["A", "B"].map((id) => ({
    id, name: id, updatedAt: "", config: persistentConfig({ ...bookConfig, url: bookConfig.url + id }),
  }));
  for (const legacy of [false, true]) {
    ...
    assert.equal(values.get("crawlspace-rule-v1"), JSON.stringify(profiles[1].config));
...
test("concurrent same-origin requests wait, other origins proceed, cancellation frees no reserved slot", async () => {
  ...
  const pace = createOriginScheduler(() => now, (ms, signal) => new Promise<void>((resolve, reject) => {
  ...
  assert.deepEqual(waits.map((w) => w.ms), [2000, 1000]);
  now = 2000;
  waits.shift()!.resume();
  await second;
  waits.shift()!.resume();
...
function streamRequest(stream: ReadableStream<Uint8Array>, signal?: AbortSignal) {
...
test("configuration upload times out even when stream cancellation never settles", async () => {
  ...
  await assert.rejects(readConfigBody(request, 20), (error: any) => error.status === 408);
  assert.equal(cancelled, true);
});

test("configuration upload enforces size, abort, and valid JSON", async () => {
  const oversized = streamRequest(new ReadableStream({
    start(controller) { controller.enqueue(new Uint8Array(32_769)); },
    cancel() { return new Promise(() => {}); },
  }));
  await assert.rejects(readConfigBody(oversized), (error: any) => error.status === 413);
  const aborter = new AbortController();
  const pending = readConfigBody(streamRequest(new ReadableStream(), aborter.signal));
  aborter.abort();
...
test("slow uploads occupy the three task slots and abort releases them", async () => {
  ...
  const requests = aborters.map((a) => POST(streamRequest(new ReadableStream(), a.signal)));
  try {
    await delay(0);
    const rejected = await POST(new Request("http://localhost/api/crawl", { method: "POST", body: "{}" }));
    assert.equal(rejected.status, 429);
  } finally {
    aborters.forEach((a) => a.abort());
    assert.deepEqual((await Promise.all(requests)).map((r) => r.status), [408, 408, 408]);
  }
  const next = await POST(new Request("http://localhost/api/crawl", { method: "POST", body: "{}" }));
