...
try {
  ...
  await page
    .getByLabel("接口网址")
    .fill("https://jsonplaceholder.typicode.com/posts?userId=1");
  await page.getByLabel("数据列表路径").fill("$");
  await page.getByLabel("站点规则名称").fill("JSON API test");
  await page
    .getByLabel(/^自定义请求头/)
    .fill('{"Authorization":"Bearer NOT-A-REAL-SECRET"}');
  await page.getByRole("button", { name: "保存规则", exact: true }).click();
  await page.getByRole("button", { name: "规则已保存" }).waitFor();
  const apiId = await page
    .getByLabel("已保存的站点", { exact: true })
    .inputValue();
  ...
  await run(10);
  console.log(
    "JSON_GET_OK: 10 real API records, invalid headers blocked, credentials not persisted",
  );

  await page.getByLabel("请求方式").selectOption("POST");
  await page
    .getByLabel("接口网址")
    .fill("https://jsonplaceholder.typicode.com/posts");
  // JSONPlaceholder documents that POST responses are simulated and do not persist server data.
  await page
    .getByLabel("JSON 请求体")
    .fill('{"title":"Crawlspace integration check","userId":1}');
  await run(1);
  assert.match(
    await page.locator(".table-scroll tbody").innerText(),
    /Crawlspace integration check/,
  );
  assert.match(await page.locator(".table-scroll tbody").innerText(), /101/);
  console.log(
    "JSON_POST_OK: simulated public testing API returned the JSON body correctly",
  );
  await page.evaluate(() => window.scrollTo(0, 0));
  await page.screenshot({
    path: path.join(output, "multi-site-api.png"),
    fullPage: true,
  });
  ...
  assert.deepEqual(
    JSON.parse(await page.getByLabel(/^自定义请求头/).inputValue()),
    {},
  );
  assert.equal(
    await page.getByLabel("接口网址").inputValue(),
    "https://jsonplaceholder.typicode.com/posts",
  );
  await page.getByRole("button", { name: "删除已保存的站点规则" }).click();
  assert.equal(
    await page.locator(`#saved-profile option[value="${apiId}"]`).count(),
    0,
  );
  ...
  await page.waitForFunction(() =>
    document.querySelector(".config-footer")?.textContent.includes("已停止"),
  );
  console.log(
    "PROFILES_AND_CANCEL_OK: independent save/load/delete, restore without credentials, stop action",
  );
  ...
  await page
    ...
    .getByRole("button", { name: /采集结果/ })
    .click();
  await page
    .locator(".mobile-navigation")
    .getByRole("button", { name: "采集工作台" })
    .click();
  await page.evaluate(() => window.scrollTo(0, 0));
  await page.screenshot({
    path: path.join(output, "multi-site-mobile.png"),
    fullPage: true,
  });
  ...
  await disclaimer
    .getByRole("heading", { name: "使用说明与免责声明", exact: true })
    .waitFor();
  assert.ok(
    await disclaimer
      .getByRole("heading", { name: /自定义接口与凭据/ })
      .isVisible(),
  );
  assert.deepEqual(errors, []);
  console.log(
    "BROWSER_FEATURES_OK: mobile navigation/layout, disclaimer route, no page errors",
  );
