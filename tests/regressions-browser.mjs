...
try {
  ...
  await page.waitForFunction(
    () =>
      ...
      [...document.querySelectorAll("input")].some(
        (i) => i.value === "https://quotes.toscrape.com/",
      ),
  );
  assert.equal(
            ...
            rows: [
              {
                名言: "Example",
                作者: "",
                _source: "https://quotes.toscrape.com/",
              },
            ],
          },
  ...
  assert.deepEqual(result.warnings, [warning, "第二页读取失败"]);
  assert.equal(result.rows.length, 1);
  await page.screenshot({
    path: path.join(output, "regressions-desktop.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(output, "regressions-mobile.png"),
    fullPage: true,
  });
  await page.goto(base + "/disclaimer");
  await page.getByRole("heading", { name: "5. 结果完整性" }).waitFor();
  assert.ok(
    (await page.locator("main").innerText()).includes("依法不得排除的责任"),
  );
  assert.ok(
    await page.evaluate(
      () => document.documentElement.scrollWidth <= innerWidth,
  ...
  await page.screenshot({
    path: path.join(output, "disclaimer-mobile.png"),
    fullPage: true,
  });
  assert.deepEqual(errors, []);
  console.log(
    "PASS: independent profile restore, warnings in partial JSON export, disclaimer and mobile layout",
  );
} finally {
  await browser.close();
}
