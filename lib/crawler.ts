...
import type { CrawlConfig, CrawlEvent, CrawlResult } from "./types";
...
export async function crawl(
  ...
  const checkRobots = async (url: URL) => {
    ...
    crawlDelay = Math.max(1_000, (seconds ?? 0) * 1000);
  };
  ...
  for (let index = 0; current && index < config.maxPages; index++) {
    ...
    const beforeRequest = async (target: URL) => {
      ...
      await pace(target.origin, crawlDelay, signal);
    };
    ...
    if (json) publicUrl.search = "";
    const parsed = json
      ? extractJson(response.body, config, publicUrl.href)
      : extractPage(response.body, config, response.url, charset);
    let atLimit = false;
